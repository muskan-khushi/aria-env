"""
ARIA — Reward Function Tests
Edge cases for the reward engine.
"""
import sys
...
class TestRewardEngine:
    ...
    def test_correct_gap_reward(self):
        ...
        assert r >= 0.20  # base + possibly severity bonus

    def test_false_positive_penalty(self):
        obs, env = make_obs()
        _, r, _, _ = env.step(ARIAAction(
            action_type=ActionType.IDENTIFY_GAP,
            clause_ref="privacy_policy.s2",        # red herring
    ...
    def test_request_section_neutral(self):
        obs, env = make_obs()
        _, r, _, _ = env.step(ARIAAction(
    ...
    def test_phase_penalty_for_remediation_before_findings(self):
        ...
        # Just verify the episode continues normally
        assert obs3.steps_taken == 2
    ...
    def test_correct_conflict_reward(self):
        ...
        _, r, _, _ = env.step(ARIAAction(
            ...
            conflict_desc="GDPR requires opt-in; CCPA allows opt-out for same processing",
        ))
        assert r == pytest.approx(0.18, abs=0.01)
